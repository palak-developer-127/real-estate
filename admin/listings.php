...
function updateStatus($conn, $listing_id, $status)
{
   $update_status = $conn->prepare("UPDATE `property` SET status = ? WHERE id = ?");
   $update_status->execute([$status, $listing_id]);
}

// Check if the status change button is clicked
if (isset ($_POST['toggle_status'])) {
   $toggle_id = $_POST['toggle_id'];
   $toggle_id = filter_var($toggle_id, FILTER_SANITIZE_STRING);

   // Retrieve the current status
   $current_status = $conn->prepare("SELECT status FROM `property` WHERE id = ?");
   $current_status->execute([$toggle_id]);
   $status_row = $current_status->fetch(PDO::FETCH_ASSOC);
   $status = $status_row['status'];

   // Toggle the status
   $new_status = ($status == 'E') ? 'D' : 'E';

   // Update the status in the database
   updateStatus($conn, $toggle_id, $new_status);
}
        ...
        <div class="box-container">
         ...
         if ($select_listings->rowCount() > 0) {
            ...
            <div class="box">
                ...
                <h3 class="name">
                    <?= $fetch_listing['property_name']; ?>
                </h3>
                <p class="location"><i class="fas fa-map-marker-alt"></i>
                    <?= $fetch_listing['address']; ?>
                </p>
                <form action="" method="POST">
                    <a href="update_property.php?get_id=<?= $listing_id; ?>" class="option-btn">Update property</a>
                </form>
                <form action="" method="POST">
                ...
                <form action="" method="POST">
                    <input type="hidden" name="toggle_id" value="<?= $listing_id; ?>">
                    <?php
                     // Display different text and color based on status
                     $status_text = ($fetch_listing['status'] == 'E') ? 'Unapprove' : 'Approve';
                     $btn_class = ($fetch_listing['status'] == 'E') ? 'btn-success' : 'btn-danger';
                     ?>
                    <button type="submit" class="btn <?= $btn_class; ?>" name="toggle_status">
                        <?= $status_text; ?>
                    </button>
                </form>
            </div>
